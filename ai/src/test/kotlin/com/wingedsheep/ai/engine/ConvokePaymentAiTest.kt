...
import com.wingedsheep.mtg.sets.definitions.ecl.cards.MerrowSkyswimmer
...
class ConvokePaymentAiTest : FunSpec({
    fun driver(): GameTestDriver = GameTestDriver().apply {
        registerCards(TestCards.all + listOf(UnexpectedAssistance, LoftyDreams, MerrowSkyswimmer))
    ...
    test("AI convokes hybrid pips when the lands cannot pay them (Merrow Skyswimmer)") {
        // {3}{W/U}{W/U} with one Plains, two Forests and two white creatures: castable only if the
        // creatures pay the hybrids and the lands the generic.
        val driver = driver()
        val player = driver.activePlayer!!
        driver.putLandOnBattlefield(player, "Plains")
        repeat(2) { driver.putLandOnBattlefield(player, "Forest") }
        repeat(2) { driver.putCreatureOnBattlefield(player, "Savannah Lions") }
        val spell = driver.putCardInHand(player, "Merrow Skyswimmer")
        val legal = LegalActionEnumerator.create(driver.cardRegistry).enumerate(driver.state, player)
            .single { (it.action as? CastSpell)?.cardId == spell }
